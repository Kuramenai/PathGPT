...
from data_augmentation import save_file
...
def calculate_dynamic_threshold(
    ...
    for path_collection in tqdm(
        ...
        sim_original = compute_jaccard_similarity(custom_path, historical_path, edge_lengths)
    ...
    for delimiter in [" - ", "-", "·", "/"]:
        if delimiter in chosen_name:
            ...
            for part in parts:
                ...
                if (prefer_chinese and has_chinese) or (not prefer_chinese and not has_chinese):
...
def get_path_with_edges_names(edge_path: List[int], edge_id_to_edge_name: Dict[int, str]) -> List[str]:
    """
...
def filter_dataset(
    ...
    filtered_dataset = []
    for path_collection in tqdm(dataset, total=len(dataset), desc="Filtering dataset", dynamic_ncols=True):
        custom_path = path_collection[f"{variables.path_type}_path_edges"]
        historical_path = path_collection["historical_path_edges"]
        fastest_path = path_collection["fastest_path_edges"]
        shortest_path = path_collection["shortest_path_edges"]
        ...
        path_collection["historical_path_edges_names"] = historical_path_with_edges_names
        ...
        path_collection[f"{custom_path_type}_path_edges_names"] = custom_path_with_edges_names
...
if __name__ == "__main__":
    ...
    try:
        ...
        cprint(f"Train data not found at {train_data_filename}! Please run data_augmentation.py first.", "red")
        exit(1)
    ...
    try:
        ...
        cprint(f"Test data not found at {test_data_filename}! Please run data_augmentation.py first.", "red")
        exit(1)
