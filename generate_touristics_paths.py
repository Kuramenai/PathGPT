...
def augment_data(
    ...
    for path_index, (
        ...
        touristic_path_with_road_names = get_path_with_road_names(
            touristic_path, road_names
        )
        ...
        if (
            ...
            and len(touristic_path_with_road_names) >= 3
        ):
            # fmt: off
            path_collection = {}
            ...
            path_collection["touristic_path_road_names"] = touristic_path_with_road_names 
...
if __name__ == "__main__":
    ...
    # We'll create a 50 meter radius circle around each POI.
    poi_buffer = pois_proj.geometry.buffer(50)

    # Use a spatial join to efficiently find all edges that intersect these zones.
    # It returns the edges that are within our 50 meter buffer of a POI.
    ...
    # Now, we update the touristic edges to True.
    ...
    augmented_train_data, neuromlr_train_data = augment_data(
        data=train_data, road_names=road_names, n_cores=os.cpu_count()
    )
    augmented_test_data, neuromlr_test_data = augment_test_data(
        data=test_data, road_names=road_names, n_cores=os.cpu_count()
    )
    ...
    if test_dataset_dissimilarities > 0:
        cprint(
            f"Incoherence between paths with roads names extracted from edges ids ans paths with street names extracted from nodes ids.\Found {test_dataset_dissimilarities} dissimilarities in test data.",
            "red",
        )
    else:
        cprint("Sanity check passed for train dataset", "green")
    cprint(f"Touristic edges: {touristic_edges_count}", "red")
    save_file("train_data/touristic_paths", file=augmented_train_data)
    save_file("test_data/touristic_paths", file=augmented_test_data)
    save_file("neuromlr_test_data/touristic_paths", file=neuromlr_test_data)
    save_file("neuromlr_train_data/touristic_paths", file=neuromlr_train_data)
